...
figure;
plot(meas(idx1, 3), meas(idx1, 4), 'r.'); hold on; % setosa는 빨간색 점으로
plot(meas(idx2, 3), meas(idx2, 4), 'go'); hold on; % versicolor는 녹색 동그라미로
plot(meas(idx3, 3), meas(idx3, 4), 'bx'); hold on; % virginica는 파란색 x로
xlabel('Pepal length');
ylabel('Pepal width');
